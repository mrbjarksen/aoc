defmodule Beam do
  ...
  def energize(contraption, start, num_rows, num_cols) do
    fire(contraption, [start], MapSet.new(), num_rows, num_cols)
...
starts =
  Enum.map(0..num_rows - 1, &({&1, 0, :right}))
  ++ Enum.map(0..num_cols - 1, &({num_rows - 1, &1, :up}))
  ++ Enum.map(0..num_rows - 1, &({&1, num_cols - 1, :left}))
  ++ Enum.map(0..num_cols - 1, &({0, &1, :down}))
...
Enum.map(starts, &Beam.energize(contraption, &1, num_rows, num_cols) |> MapSet.size())
|> Enum.max()
|> IO.puts()
